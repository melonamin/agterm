...
struct ControlDispatcherTests {
    ...
    @Test func workspaceFocusRoutesModeAndRejectsInvalidMode() {
        let actions = MockControlActions()
        let dispatcher = ControlDispatcher(actions: actions)
        ...
        let bad = dispatcher.dispatch(ControlRequest(
            cmd: .workspaceFocus,
            target: "workspace",
            args: ControlArgs(mode: "bogus")
        ))

        #expect(focused == ControlResponse(ok: true))
        #expect(bad == ControlResponse(ok: false, error: "invalid focus mode: bogus"))
        #expect(actions.calls == [.workspaceFocus(target: "workspace", window: "win", .on)])
    }

    @Test func sessionFlagRoutesModeAndRejectsInvalidMode() {
        let actions = MockControlActions()
        let dispatcher = ControlDispatcher(actions: actions)
        ...
        let bad = dispatcher.dispatch(ControlRequest(cmd: .sessionFlag, args: ControlArgs(mode: "bogus")))

        #expect(flagged == ControlResponse(ok: true))
        #expect(cleared == ControlResponse(ok: true))
        #expect(bad == ControlResponse(ok: false, error: "invalid flag mode: bogus"))
        #expect(actions.calls == [
    ...
    @Test func splitScratchFocusAndResizeRouteParsedInputs() {
        ...
        #expect(actions.calls == [
            ...
            .sessionFocus(target: "session", window: nil, .split),
            .sessionResize(target: "session", window: "win", .delta(-0.1))
        ])
    }

    @Test func splitScratchFocusAndResizeRejectInvalidInputs() {
        let actions = MockControlActions()
        let dispatcher = ControlDispatcher(actions: actions)

        let split = dispatcher.dispatch(ControlRequest(cmd: .sessionSplit, args: ControlArgs(mode: "bogus")))
        let scratch = dispatcher.dispatch(ControlRequest(cmd: .sessionScratch, args: ControlArgs(mode: "bogus")))
        let focus = dispatcher.dispatch(ControlRequest(cmd: .sessionFocus, args: ControlArgs(pane: "bogus")))
        ...
        #expect(split == ControlResponse(ok: false, error: "invalid split mode: bogus"))
        #expect(scratch == ControlResponse(ok: false, error: "invalid scratch mode: bogus"))
        #expect(focus == ControlResponse(ok: false, error: "invalid pane: bogus"))
...
private final class MockControlActions: ControlActions {
    enum Call: Equatable {
        ...
        case workspaceFocus(target: String?, window: String?, ControlToggleMode)
        case sessionFlag(target: String?, window: String?, ControlSessionFlagMode)
        case sessionStatus(target: String?, window: String?, ControlSessionStatusUpdate)
        case sessionSplit(target: String?, window: String?, ControlToggleMode)
        case sessionScratch(target: String?, window: String?, ControlToggleMode, command: String?)
        case sessionFocus(target: String?, window: String?, ControlPaneFocusMode)
    ...
    func splitSession(_ target: String?, window: String?, mode: ControlToggleMode) -> ControlResponse {
        calls.append(.sessionSplit(target: target, window: window, mode))
        return ControlResponse(ok: true)
    }

    func scratchSession(_ target: String?, window: String?, mode: ControlToggleMode,
                        command: String?) -> ControlResponse {
        calls.append(.sessionScratch(target: target, window: window, mode, command: command))
        return ControlResponse(ok: true)
    }

    func focusSessionPane(_ target: String?, window: String?, pane: ControlPaneFocusMode) -> ControlResponse {
