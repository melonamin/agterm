...
public protocol ControlActions {
    ...
    func scratchSession(_ target: String?, window: String?, mode: ControlToggleMode, command: String?) -> ControlResponse
    ...
    public func dispatch(_ request: ControlRequest) -> ControlResponse? {
        ...
        case .workspaceFocus:
            guard let mode = ControlToggleMode.parse(request.args?.mode) else {
                return ControlResponse(ok: false, error: "invalid focus mode: \(request.args?.mode ?? "toggle")")
            }
            return actions.focusWorkspace(request.target, window: request.args?.window, mode: mode)
        case .sessionFlag:
            guard let mode = ControlSessionFlagMode.parse(request.args?.mode) else {
                return ControlResponse(ok: false, error: "invalid flag mode: \(request.args?.mode ?? "toggle")")
            }
            return actions.setSessionFlag(request.target, window: request.args?.window, mode: mode)
        case .sessionStatus:
        ...
        case .sessionSplit:
            guard let mode = ControlToggleMode.parse(request.args?.mode) else {
                return ControlResponse(ok: false, error: "invalid split mode: \(request.args?.mode ?? "toggle")")
            }
            return actions.splitSession(request.target, window: request.args?.window, mode: mode)
        case .sessionScratch:
            guard let mode = ControlToggleMode.parse(request.args?.mode) else {
                return ControlResponse(ok: false, error: "invalid scratch mode: \(request.args?.mode ?? "toggle")")
            }
            return actions.scratchSession(request.target, window: request.args?.window, mode: mode,
                                          command: request.args?.command)
        case .sessionFocus:
            guard let pane = ControlPaneFocusMode.parse(request.args?.pane) else {
                return ControlResponse(ok: false, error: "invalid pane: \(request.args?.pane ?? "other")")
            }
            return actions.focusSessionPane(request.target, window: request.args?.window, pane: pane)
        case .sessionResize:
