...
struct Window: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        subcommands: [New.self, List.self, Select.self, Close.self, Rename.self, Delete.self, Resize.self,
                      Move.self, Zoom.self, Fullscreen.self]
    )
    ...
    struct Fullscreen: RequestCommand {
        static let configuration = CommandConfiguration(abstract: "Toggle native full screen for a window.")
        @Argument(help: "Window id, unique prefix, or 'active'.") var id: String = "active"
        @OptionGroup var options: BasicOptions
