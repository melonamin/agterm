...
struct BasicOptions: ParsableArguments, ConnectionOptions {
    ...
    /// Resolve the socket path: explicit `--socket`, else the same Linux rendezvous path as the GTK app.
...
struct SurfaceTargetOptions: ParsableArguments {
    @Option(name: .long, help: "Target surface id from tree, 'quick', or 'active'.")
    var target: String = "active"
}
